...
class InventoryServiceClientTest {
    ...
    void testGetAllProductsAsync_Success() {
        ...
        // Act
        List<ProductDTO> products = inventoryServiceClient.getAllProductsAsync().block();

        // Assert
        assertNotNull(products);
        assertEquals(1, products.size());
        assertEquals("Async Product", products.get(0).getName());
        assertEquals(79.99, products.get(0).getPrice());
    }

    @Test
    void testGetAllProductsAsync_ServerError_ReturnsEmptyList() {
        // Arrange
        mockWebServer.enqueue(new MockResponse()
                .setResponseCode(500)
        ...
        // Act - async method returns empty list on error
        List<ProductDTO> products = inventoryServiceClient.getAllProductsAsync().block();
